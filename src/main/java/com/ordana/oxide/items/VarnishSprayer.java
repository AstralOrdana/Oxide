        ...
        //implements IThirdPersonAnimationProvider //TODO: add this for fancy animation
{
    ...
    public void onUseTick(Level level, LivingEntity livingEntity, ItemStack stack, int remainingUseDuration) {
        ...
        for (int y = -3; y < 3; ++y) {
            for (int x = -3; x < 3; ++x) {
                ...
                fluidDrop.shootFromRotation(livingEntity, livingEntity.getXRot() + (y * 5 * level.getRandom().nextFloat()), livingEntity.getYRot() + (x * 5 * level.getRandom().nextFloat()), 1.0F + (5 * level.getRandom().nextFloat()), 1.0F + (level.getRandom().nextFloat() / 2), 1.0F);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> builder, TooltipFlag tooltipFlag) {
        ...
        if (PlatHelper.getPhysicalSide().isClient()) {
            ...
            fluid.addToTooltip(context, builder, tooltipFlag, stack);
        }
    }
    ...
    public int getBarColor(ItemStack stack) {
        ...
        SFStackView fluid = getFluidComponent(stack, clientLevel.registryAccess());
        if (fluid.isEmpty()) return -1;
        return OxideClient.getParticleColor(fluid.toMutable(), clientLevel, BlockPos.ZERO);
    }
